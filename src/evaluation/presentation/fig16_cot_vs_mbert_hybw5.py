"""Fig 16 - Baseline CoT vs MentalBERT+CORN+Hybrid-W5, per PHQ-8 item.
...
Head-to-head between the two standalone models (no cascade/ensemble):
  ...
  * "MentalBERT+W5"   = the trained encoder, MentalBERT+CORN+Hybrid-W5
                        (outputs/cv/oof_predictions_ctxm_corn_hybw5.csv)
Same pooled out-of-fold slice (n=1752), matched on (participant_id, item_id).
...
MAE / QWK for both models.
    ...
    python -m src.evaluation.presentation.fig16_cot_vs_mbert_hybw5
"""
...
ENC_OOF = S.ROOT / "outputs" / "cv" / "oof_predictions_ctxm_corn_hybw5.csv"
LAB = S.LABELS
KEY = ["participant_id", "item_id"]

COT_LABEL = "Baseline CoT (Qwen2.5-7B, greedy, Hybrid-W3)"
ENC_LABEL = "MentalBERT+CORN+Hybrid-W5"
...
def main():
    ...
    ax0.set_title("Per-item macro-F1: baseline CoT vs MentalBERT+CORN+Hybrid-W5  (★ = winner)",
    ...
    S.save(fig, "fig16_cot_vs_mbert_hybw5")
    ...
    cp = S.companion_path("fig16_cot_vs_mbert_hybw5", "csv")
