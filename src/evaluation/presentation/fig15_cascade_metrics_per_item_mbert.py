"""Fig 15b - Cascade family metrics, per PHQ-8 item (MentalBERT+CORN+Hybrid-W3).

Same four configs and four metrics as fig14 (macro-F1, QWK, MAE, err>=2), but
broken down by symptom instead of pooled -- using the MentalBERT+CORN+Hybrid-W3
encoder for the encoder-dependent configs (instead of the Attention-MIL encoder
used in fig15). Shows WHERE the merged-gate cascade's win comes from with this
encoder, and which symptoms still resist every configuration.

All from pooled out-of-fold predictions (n=1752, ~219/item); no GPU.

    python -m src.evaluation.presentation.fig15_cascade_metrics_per_item_mbert
"""
...
ENC_TAG = "ctxm_corn_hybw3"   # MentalBERT + CORN + Hybrid-W3
LAB = S.LABELS

CONFIGS = [
    ("LLM alone", "llm", S.CALM_COT),
    ("Encoder · MentalBERT+CORN+W3", "enc", S.CALM_ENC),
...
def main():
    ...
    fig.suptitle("Per-symptom breakdown: LLM / Encoder(MentalBERT+CORN+W3) / Cascade metrics",
                 fontsize=15.5, color=S.ACCENT, y=1.06)
    fig.text(0.5, -0.03,
             "Pooled out-of-fold (n=1752, ~219/item). Encoder = MentalBERT+CORN+Hybrid-W3. "
             "Appetite/Sleep/Moving stay hard for every configuration -- the residual ceiling "
             "is intrinsic to those symptoms, not the retrieval or cascade design.",
             ha="center", fontsize=9.5, color=S.MUTED)

    S.save(fig, "fig15_cascade_metrics_per_item_mbert", tight=False)

    cp = S.companion_path("fig15_cascade_metrics_per_item_mbert", "json")
